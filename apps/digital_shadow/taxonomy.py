...
# ── Человекочитаемые описания сигналов (объяснимость для аналитика, §0) ───────
# Зачем: каждая находка должна объяснять, ПОЧЕМУ помечена — для ручного реагирования.
# Покрывает шадоу-сигналы и общие сигналы движка (core), которые могут всплыть в находке.
SIGNAL_DESCRIPTIONS: dict[str, str] = {
    # шадоу-специфика
    "darknet_listing": "Источник — даркнет/закрытая площадка (.onion/leak-форум)",
    "contraband_keyword": "Лексика контрабанды (вейпы/алкоголь оптом, «без акциз»)",
    "drug_slang": "Наркотическая лексика/сленг (закладки, клад, реагент)",
    "drop_recruitment": "Признаки вербовки дропов (карта в аренду, приём переводов, обнал)",
    "kz_data_leak": "Признаки утечки/продажи базы данных РК (дамп/слив + KZ-привязка)",
    "iin_dump_mention": "Упоминание дампа с ИИН/ПДн (ФИО, паспорт, телефоны)",
    "bad_crypto_wallet": "Криптокошелёк в списке известных «плохих» адресов (badlist/abuse-фид)",
    "mixer_or_tumbler": "Признаки миксера/тумблера — отмыв криптовалюты",
    "crypto_only_payment": "Оплата только в криптовалюте (USDT/BTC/TRC20)",
    "encrypted_contact": "Увод контакта в закрытый канал (session/threema/jabber/telegram)",
    "bulk_sale": "Оптовые объёмы продажи («оптом», wholesale, көтерме)",
    "no_kyc": "«Без документов / без верификации / анонимно»",
    "counterfeit": "Контрафакт/подделка (реплика, копия AAA)",
    "document_forgery": "Поддельные документы (диплом/справка/удостоверение под заказ)",
    "watchlisted": "Сущность из watchlist аналитика — на отслеживании",
    "known_bad_entity": "Индикатор с подтверждённым abuse (репутация-flywheel)",
    "similar_to_known_listing": "Семантически близко к известному «плохому» листингу (Qdrant)",
    "ml_category": "Категория предложена ML-классификатором (правила дали unknown)",
    # общий движок (core) — то, что встречается в теневых находках
    "graph_entity_reuse": "Сущность повторяется в нескольких источниках (скрытая связь в графе)",
    "crypto_wallet_found": "В тексте найден криптокошелёк",
    "crypto_payment": "Упоминание оплаты в криптовалюте",
    "money_mule_request": "Признаки вербовки денежного мула (приём/вывод средств)",
    "telegram_contact": "Увод контакта в Telegram",
    "suspicious_domain": "Подозрительный домен",
    "promo_code_found": "В тексте найден промокод",
    "casino_domain_found": "Упомянут домен онлайн-казино",
    "phishing_url": "Подозрительная/фишинговая ссылка",
    "similar_to_known_scam": "Семантически близко к известному скам-кейсу",
}


def describe_signal(signal: str) -> str:
    """Человекочитаемое объяснение сигнала (для evidence/UI). Неизвестный → сам код."""
    return SIGNAL_DESCRIPTIONS.get(signal, signal)
