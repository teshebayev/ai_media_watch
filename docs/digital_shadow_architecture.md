...
|---|---|---|
| Коллекторы | `collectors/{file_collector,http_page,rss,paste_sites,darknet_mock}.py` | источник → `RawItem`; Tor через `--proxy`; paste — фильтр по утечкам БД РК |
| Дедуп/инкремент | `seen_store.py` | seen-store (id + контентный хэш) → собираем только новое (`collect.py`) |
...
**Анти-обфускация (Фаза 3):** `normalize_text` снимает разрядку («к л а д»→«клад»),
гомоглифы (лат.↔кир.: «kлад»/«vейп»→«клад»/«вейп») и повторы. Скан идёт по оригиналу
И нормализованному тексту (объединение) — без регрессий word-boundary.
**Семантика (Qdrant):** коллекция `shadow_listings` (наполняется `make shadow-index`);
новый элемент → nearest-neighbor → при cos-sim ≥ 0.8 к известному `bad` → сигнал
`similar_to_known_listing` (+25). Ловит перефразировки. Best-effort: нет Qdrant/эмбеддера → skip.
**Объяснимость:** `ShadowFinding.evidence` = совпавшие слова лексикона + топ-3 вклада в
скоринг (`shadow_score.breakdown`) — аналитик видит «почему»; отражено в UI «Находки».
...
| `POST /shadow/collect/demo` | демо-датасет с общими индикаторами → кластеры по торговлям (вейпы/алко/наркотики/дропы/утечки) + кросс-категорийный мост |
...
| `GET /shadow/actors/scored` | риск-скоринг акторов: охват × степень в сети (co_actors) × разнообразие × кросс-продукт |
| `GET /shadow/clusters` | кластеры связанных акторов + `risk`, `hub` («вожак»), `categories` и `cross_category` (общий индикатор связывает наркотики↔дропы↔утечки) |
| `GET /shadow/path?a=&b=` | объяснимость связи: кратчайший путь между двумя сущностями + «почему связаны» |
| `GET /shadow/signals` | справочник риск-сигналов (код · вес · человекочитаемое описание) — легенда |
...
с флагом `cross_product` (мост Media↔Shadow). **Скоринг акторов** (`/shadow/actors/scored`):
риск отдельной сущности = охват (`uses`) × степень в сети (`co_actors` — с сколькими др.
сущностями делит источник) × разнообразие классов источников × кросс-продукт — приоритизирует
*какой именно* кошелёк/домен/@ник центральный, а не просто частый. **Кластеры**
(`/shadow/clusters`): связные компоненты по со-упоминанию (union-find над двудольными рёбрами
«сущность ← источник») — видна структура сети, кросс-продуктовые кластеры наверху. Каждый кластер
получает агрегированный `risk` сети (охват × объём инфраструктуры × разнотипность индикаторов ×
**кросс-категорийность** × кросс-продукт) и `hub` — «вожак» сети (сущность с максимальной
степенью), что выделяет ядро сети дропов/кошельков для приоритетного реагирования.
**Кросс-категория** (`cross_category`, `categories`): источники несут шадоу-категорию в узле
графа (`ShadowItem.category`), поэтому если ОДИН кошелёк/телеграм/домен всплывает и в листинге
наркотиков, и в вербовке дропов, и в продаже утечки — кластер/актор помечается как
кросс-категорийный (главный сигнал организованной сети) и поднимается наверх очереди.
**Объяснимость пути** (`/shadow/path?a=&b=`, `graph_service.explain_link`): кратчайший путь
между двумя сущностями + человекочитаемое «почему связаны» (напр. «обе упоминаются в одном
источнике [drop_recruitment]») — аналитик видит основание связи, а не только факт.
**Извлечение кошельков** покрывает BTC/ETH **и TRON/TRC20** (USDT-TRC20 — типичная оплата в
KZ крипто-преступности), иначе теневые кошельки не доходили бы до графа. Все ключи сущностей канонизируются
